...
public class BeerCommandController {
    ...
    public boolean createBeer(@RequestBody CreateBeerRestModel beer){
            ...
            CreateBeerCommand command = CreateBeerCommand.builder()._id(UUID.randomUUID().toString()).name(beer.getName())
                    .description(beer.getDescription()).ingredients(beer.getIngredients()).methods(beer.getMethods()).build();
